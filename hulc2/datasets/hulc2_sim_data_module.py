...
class Hulc2SimdDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, *args, **kwargs):
        ...
        if self.use_shm:
            train_shmem_loader = SharedMemoryLoader(self.datasets_cfg, self.training_dir, split="training")
            train_shm_lookup = train_shmem_loader.load_data_in_shared_memory()

            val_shmem_loader = SharedMemoryLoader(self.datasets_cfg, self.val_dir, split="validation")
    ...
    def setup(self, stage=None):
        transforms = load_dataset_statistics(
            self.root_data_path / "training", self.root_data_path / "validation", self.transforms
        )
        ...
        for _, dataset in self.datasets_cfg.items():
            train_dataset = hydra.utils.instantiate(
                dataset, datasets_dir=self.training_dir, transforms=self.train_transforms, split="training"
            )
            val_dataset = hydra.utils.instantiate(
                dataset, datasets_dir=self.val_dir, transforms=self.val_transforms, split="validation"
            )
            if self.use_shm:
