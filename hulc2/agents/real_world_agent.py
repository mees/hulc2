...
from hulc2.utils.utils import add_img_text, get_abspath, get_aff_model, resize_pixel
...
class AffHULCAgent:
    ...
    def crop_and_resize_pixel(self, px, full_res_img):
        ...
        if crop_coords is not None:
            ...
            px[0] = px[0] - c[0]
            px[1] = px[1] - c[2]
        resize_px = resize_pixel(px, img.shape[:2], self.static_cam.get_resize_res())
        return resize_px
    ...
    def get_aff_pred(self, caption, obs, out_shape=None):
        ...
        out_img, _info = self.point_detector.get_preds_viz(inp, pred, out_shape=out_shape)
