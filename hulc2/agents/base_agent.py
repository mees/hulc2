...
from hulc2.utils.utils import add_img_text, get_aff_model, resize_pixel
...
class BaseAgent:
    def __init__(self, env, offset, aff_cfg, viz_obs=False, save_viz=False, *args, **kwargs):
        ...
        self.save_dir = {"parent": save_directory, "sequence_counter": 0, "rollout_counter": 0, "step_counter": 0}
        self.sequence_data = {}
    ...
    def reset_position(self):
        return self.move_to(self.origin, self.target_orn, 1)
    ...
    def save_sequence_txt(self, filename, data):
        output_dir = os.path.join(self.save_dir["parent"], "seq_%03d" % self.save_dir["sequence_counter"])
    ...
    def save_img(self, img, folder="./", name="img"):
        outdir = os.path.join(self.save_dir["parent"], "seq_%03d" % self.save_dir["sequence_counter"])
        outdir = os.path.join(outdir, "task_%02d" % self.save_dir["rollout_counter"])
    ...
    def move_to_pos(self, tcp_pos, action):
        ...
        ts = 0
        while (
            np.linalg.norm(error) > 0.01  # Large error
            and (
                np.linalg.norm(curr_pos - last_pos) > 0.0005  # Moving
            ...
            action = [curr_pos + rel_pos, target_orn, action[-1]]
