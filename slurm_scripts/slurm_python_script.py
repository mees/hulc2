...
def parse_args():
    ...
    parser.add_argument("--python_script", type=str, default="../hulc2/utils/create_splits.py")
    parser.add_argument("-l", "--log_dir", type=str, default=default_log_dir)
    parser.add_argument("-j", "--job_name", type=str, default="generic")
...
def main():
    ...
    args.python_script = Path(args.python_script).resolve()
    log_name = "%x.%N.%j_python"
    ...
    script = f"{args.script.as_posix()} {args.venv} {args.python_script.as_posix()} {' '.join(unknownargs)}"
